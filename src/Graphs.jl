...
"""
struct Graph{I<:Int,FT<:AbstractFloat,IA1D<:AbstractVector{I}} <: AbstractGraph
...
"""
    build_vertex_graph(mesh::AbstractMesh{I,FT}) where {I,FT}

This function builds a graph, with adjacency based on mesh elements connected by a vertex.
"""
function build_vertex_graph(mesh::AbstractMesh{I,FT}) where {I,FT}
    @unpack nelems, unique_verts, uverts_conn, uverts_offset, elem_verts = mesh
    connect = [zeros(I, i) for i in zeros(I, nelems)]
    gsize = 0
    spectral_radius = FT(0)
    for i = 1:length(unique_verts)
        st, en = uverts_offset[i:i+1]
        for j = st:2:en-1
            el = uverts_conn[j]
            for k = st:2:en-1
                elc = uverts_conn[k]
                if elc ≠ el && findfirst(connect[el] .== elc) == nothing
                    push!(connect[el], elc)
                    gsize += 1
                end
            end
    ...
    spectral_radius *= 2
    return Graph(g_nverts, g_edge_data, g_edge_offset, Array(1:g_nverts), spectral_radius)
end
"""
    build_face_graph(mesh::AbstractMesh{I,FT}) where {I,FT}

This function builds a graph, with adjacency based on mesh elements connected by a face.
"""
function build_face_graph(mesh::AbstractMesh{I,FT}) where {I,FT}
    @unpack nelems, elem_faces, face_neighbors = mesh
    nlocalfaces = size(elem_faces, 2)

    g_nverts = nelems
    g_edge_offset = Array{I}(undef, g_nverts + 1)
    g_edge_offset[1] = 1
    g_edge_data = Array{I}(undef, nlocalfaces * nelems) # conservative initial estimate

    temp = zeros(I, nlocalfaces)
    spectral_radius = FT(0)

    for el = 1:nelems
        n, st = 0, g_edge_offset[el]
        for f = 1:nlocalfaces
            fc = elem_faces[el, f]
            if face_neighbors[fc, 1] == el
        ...
        en = st + n
        g_edge_offset[el+1] = en
        g_edge_data[st:en-1] .= sort(temp[1:n])
        spectral_radius = max(spectral_radius, FT(en - st))
    end
    spectral_radius *= 2
    return Graph(g_nverts, g_edge_data, g_edge_offset, Array(1:g_nverts), spectral_radius)
...
function laplacian_x_v!(
    ...
    for i = 1:nverts
        vout[i] = FT(0)
        for j = edge_offset[i]:edge_offset[i+1]-1
...
function fiedler_vector(graph::Graph{<:Any,FT}, ::Lanczos) where {FT} # using Lanczos
    ...
    [start[i] = i^(-1.0) for i = 1:nv] # initial guess
    ...
    eigval, eigvec = LinearAlgebra.LAPACK.syevr!('V', 'I', 'L', H, vl, vu, il, iu, abstol)
    lam = sr - eigval[1]
...
function fiedler_vector(graph::Graph{<:Any,FT}, ::PowerIt) where {FT} # using power_iteration
    ...
    split = cld(nv, 2)
    ev = Vector{FT}(undef, nv)
    [ev[i] = -1.0 for i = 1:split]
    [ev[i] = 1.0 for i = split+1:nv]
...
function fiedler_vector(graph::Graph{<:Any,FT}, ::Direct) where {FT}
    ...
    #abstol = FT(1e-12)
    abstol = FT(1e-14)
    _, z = LinearAlgebra.LAPACK.syevr!('V', 'I', 'L', lmat, vl, vu, il, iu, abstol)
    return z[:]
end
...
function split_graph(graph::Graph{I,FT}, n1) where {I,FT}
    ...
    #order = sortperm(fiedler_vector_direct(graph))
    #order = sortperm(fiedler_vector_power_it(graph))
    ...
    [edge_offset1[i+1] += edge_offset1[i] for i = 1:n1]
    [edge_offset2[i+1] += edge_offset2[i] for i = 1:n2]
